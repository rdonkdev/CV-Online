<template>
  <div class="cv-page flex font-sans text-sm text-gray-700" :style="{ '--accent': cv.accent }">
    <!-- Sidebar clara -->
    <aside class="w-[32%] p-7">
      <h1 class="text-xl font-bold leading-tight text-gray-900">{{ p.name || 'O teu nome' }}</h1>
      <p class="mt-1 text-sm font-medium text-accent">{{ p.title }}</p>

      <div class="mt-6">
        <h2 class="side-title">Contacto</h2>
        <ul class="space-y-1 text-xs text-gray-600">
          <li v-for="(c, i) in contact" :key="i" class="break-all">{{ c }}</li>
        </ul>
      </div>

      <div v-if="cv.sections.skills && cv.skills.length" class="mt-6">
        <h2 class="side-title">Competências</h2>
        <div v-for="s in cv.skills" :key="s.id" class="mb-2 break-inside-avoid">
          <p class="mb-0.5 text-xs">{{ s.name }}</p>
          <div class="h-1.5 w-full rounded-full bg-gray-200">
            <div
              class="h-full rounded-full bg-accent"
              :style="{ width: `${(s.level / 5) * 100}%` }"
            />
          </div>
        </div>
      </div>

      <div v-if="cv.sections.languages && cv.languages.length" class="mt-6">
        <h2 class="side-title">Idiomas</h2>
        <div
          v-for="l in cv.languages"
          :key="l.id"
          class="flex justify-between text-xs text-gray-600"
        >
          <span>{{ l.name }}</span
          ><span class="text-gray-400">{{ l.level }}</span>
        </div>
      </div>

      <div v-if="cv.sections.education && cv.education.length" class="mt-6">
        <h2 class="side-title">Educação</h2>
        <div v-for="e in cv.education" :key="e.id" class="mb-2 break-inside-avoid text-xs">
          <p class="font-semibold text-gray-900">{{ e.degree }}</p>
          <p class="text-gray-600">{{ e.field }}</p>
          <p class="text-gray-500">{{ e.institution }}</p>
          <p class="text-gray-400">{{ fmt(e.startDate) }} — {{ fmt(e.endDate) }}</p>
        </div>
      </div>
    </aside>

    <!-- Conteúdo -->
    <main class="flex-1 p-7">
      <section v-if="cv.sections.summary && p.summary" class="mb-5">
        <h2 class="main-title">Perfil</h2>
        <p class="whitespace-pre-line leading-relaxed text-gray-600">{{ p.summary }}</p>
      </section>

      <section v-if="cv.sections.experience && cv.experience.length" class="mb-5">
        <h2 class="main-title">Experiência</h2>
        <div v-for="exp in cv.experience" :key="exp.id" class="mb-4 break-inside-avoid">
          <div class="flex items-baseline justify-between">
            <span class="font-semibold text-gray-900">{{ exp.role }}</span>
            <span class="text-xs text-gray-400">
              {{ fmt(exp.startDate) }} — {{ exp.current ? 'Presente' : fmt(exp.endDate) }}
            </span>
          </div>
          <p class="mb-1 text-xs font-medium text-accent">{{ exp.company }}</p>
          <ul
            v-if="bullets(exp.description).length"
            class="cv-bullets ml-4 list-disc space-y-0.5 text-gray-600"
          >
            <li v-for="(b, bi) in bullets(exp.description)" :key="bi">{{ b }}</li>
          </ul>
        </div>
      </section>

      <section v-if="cv.sections.projects && cv.projects.length" class="mb-5">
        <h2 class="main-title">Projetos</h2>
        <div v-for="pr in cv.projects" :key="pr.id" class="mb-3 break-inside-avoid">
          <div class="flex items-baseline justify-between gap-2">
            <span class="font-semibold text-gray-900">{{ pr.name }}</span>
            <span v-if="pr.link" class="break-all text-xs text-accent">{{ pr.link }}</span>
          </div>
          <ul
            v-if="bullets(pr.description).length"
            class="cv-bullets ml-4 list-disc space-y-0.5 text-gray-600"
          >
            <li v-for="(b, bi) in bullets(pr.description)" :key="bi">{{ b }}</li>
          </ul>
        </div>
      </section>

      <section v-if="cv.sections.certifications && cv.certifications.length">
        <h2 class="main-title">Certificações</h2>
        <div
          v-for="c in cv.certifications"
          :key="c.id"
          class="mb-1 flex items-baseline justify-between break-inside-avoid"
        >
          <span class="text-gray-700"
            ><span class="font-medium text-gray-900">{{ c.name }}</span
            ><span v-if="c.issuer"> — {{ c.issuer }}</span></span
          >
          <span class="text-xs text-gray-400">{{ c.year }}</span>
        </div>
      </section>
    </main>
  </div>
</template>

<script setup>
import { useTemplateData } from '@/composables/useTemplateData'
const { cv, p, fmt, bullets, contact } = useTemplateData()
</script>

<style scoped>
.cv-page {
  /* Banda lateral clara contínua em todas as páginas. */
  background: linear-gradient(to right, #f1f5f9 0 32%, #ffffff 32% 100%);
  -webkit-print-color-adjust: exact;
  print-color-adjust: exact;
}
.side-title {
  @apply mb-2 text-xs font-bold uppercase tracking-wider;
  color: var(--accent);
}
.main-title {
  @apply mb-2 border-b border-gray-200 pb-1 text-sm font-bold uppercase tracking-wider text-gray-800;
}
</style>
