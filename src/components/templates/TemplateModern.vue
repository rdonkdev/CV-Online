<template>
  <div class="cv-page flex font-sans text-sm text-gray-800">
    ...
    <aside class="w-[34%] p-8 text-slate-100">
      ...
      <p class="mt-1 text-sm font-medium text-blue-300">{{ p.title }}</p>

      <div class="mt-8 space-y-2 text-xs text-slate-300">
        <p v-if="p.email" class="flex items-start gap-2 break-all">
          <svg class="mt-px h-3.5 w-3.5 shrink-0 text-blue-300" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="2" y="4" width="20" height="16" rx="2"/><path d="m22 6-10 7L2 6"/></svg>
          <span>{{ p.email }}</span>
        </p>
        <p v-if="p.phone" class="flex items-start gap-2">
          <svg class="mt-px h-3.5 w-3.5 shrink-0 text-blue-300" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07 19.5 19.5 0 0 1-6-6 19.79 19.79 0 0 1-3.07-8.67A2 2 0 0 1 4.11 2h3a2 2 0 0 1 2 1.72c.13.96.36 1.9.7 2.81a2 2 0 0 1-.45 2.11L8.09 9.91a16 16 0 0 0 6 6l1.27-1.27a2 2 0 0 1 2.11-.45c.9.34 1.85.57 2.81.7A2 2 0 0 1 22 16.92z"/></svg>
          <span>{{ p.phone }}</span>
        </p>
        <p v-if="p.location" class="flex items-start gap-2">
          <svg class="mt-px h-3.5 w-3.5 shrink-0 text-blue-300" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M20 10c0 6-8 12-8 12s-8-6-8-12a8 8 0 0 1 16 0z"/><circle cx="12" cy="10" r="3"/></svg>
          <span>{{ p.location }}</span>
        </p>
        <p v-if="p.linkedin" class="flex items-start gap-2 break-all">
          <svg class="mt-px h-3.5 w-3.5 shrink-0 text-blue-300" viewBox="0 0 24 24" fill="currentColor"><path d="M19 3a2 2 0 0 1 2 2v14a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h14zM8.34 17V9.99H6V17h2.34zM7.17 8.95a1.36 1.36 0 1 0 0-2.71 1.36 1.36 0 0 0 0 2.71zM18 17v-3.84c0-2.05-1.1-3-2.56-3-1.18 0-1.71.65-2 1.11V9.99H11.1V17h2.34v-3.91c0-.21.02-.41.08-.56.16-.41.54-.84 1.17-.84.82 0 1.15.62 1.15 1.54V17H18z"/></svg>
          <span>{{ p.linkedin }}</span>
        </p>
        <p v-if="p.website" class="flex items-start gap-2 break-all">
          <svg class="mt-px h-3.5 w-3.5 shrink-0 text-blue-300" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M10 13a5 5 0 0 0 7.54.54l3-3a5 5 0 0 0-7.07-7.07l-1.72 1.71"/><path d="M14 11a5 5 0 0 0-7.54-.54l-3 3a5 5 0 0 0 7.07 7.07l1.71-1.71"/></svg>
          <span>{{ p.website }}</span>
        </p>
      </div>

      <div v-if="cv.skills.length" class="mt-8">
        <h2 class="section-title text-blue-300">Competências</h2>
        <div class="space-y-3">
          <div v-for="s in cv.skills" :key="s.id">
      ...
      <div v-if="cv.education.length" class="mt-8">
        <h2 class="section-title text-blue-300">Educação</h2>
    ...
    <main class="flex-1 p-8">
      ...
      <section v-if="cv.experience.length">
        <h2 class="section-title text-slate-800">Experiência</h2>
        <div
          v-for="exp in cv.experience"
          :key="exp.id"
          class="relative mb-5 break-inside-avoid border-l-2 border-slate-200 pl-4"
        >
          <span class="absolute -left-[5px] top-1 h-2 w-2 rounded-full bg-blue-500" />
          ...
          <p class="mb-1 text-xs font-medium text-blue-600">{{ exp.company }}</p>
